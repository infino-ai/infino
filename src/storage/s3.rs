...
//!   - **Local RustFS test harness**: [`Self::new_with_endpoint`]
//!     takes the daemon's `https://127.0.0.1:<port>` endpoint
//!     plus a bucket name + test credential pair. The
//!     `supertable/storage/smoke_rustfs.rs` integration test uses
...
impl S3StorageProvider {
    ...
    pub fn new_with_endpoint(
        ...
        let store = AmazonS3Builder::new()
            ...
            // localhost-style endpoints (RustFS, MinIO,
            ...
            // S3-compatible endpoints (RustFS / MinIO): reqwest
...
mod tests {
    ...
    //! `supertable_smoke_via_rustfs_https` integration test
    //! (`INFINO_TEST_RUSTFS=1`; see `tests/supertable/storage/smoke_rustfs.rs`).
