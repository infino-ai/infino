    ...
    superfile::{
        ...
        fts::{
            ...
            reader::{
                ...
                LiveFloor, NormalizedExpansion, OR_WINDOW_MIN_TERMS, OrCursorSet, PreparedClauses,
                QueryExpansion,
            },
            tokenize::{Phrase, QueryClauses},
        },
    },
    supertable::{
        error::QueryError,
...
impl SupertableReader {
    ...
    pub(crate) async fn bm25_search_async(
        ...
        // Destructured once here rather than threaded as four
        // positionals: `mode` shapes the clause split, `stats` selects
        // the idf source, `bm25` — when set — overrides what each
        // column declared, which every per-superfile reader below has
        // to apply identically or two superfiles would score one query
        // two ways, and `expansion` is the per-call vocabulary that
        // wins over the column's registration at the parse point.
        let Bm25SearchOptions {
            mode,
            stats,
            bm25: bm25_override,
            expansion: per_call_expansion,
        } = opts;
        ...
        let per_call = match &per_call_expansion {
            None => None,
            Some(expansion) => Some(
    ...
    > {
        ...
        // The column's registered expansion applies at the parse point,
        // before the default operator resolves polarity — the same step
        // the scored path takes.
        let parsed = tokenizer.parse(query);
    ...
    pub(crate) async fn token_match_async(
        ...
        let column_arc = Arc::new(column.to_owned());
        // Frozen whole (terms, phrases and groups together): the kernel
        // below reads all three lists of each.
    ...
    pub(crate) async fn token_match_count_async(
        ...
        let atoms_involved = match_set.has_atoms() || negatives.has_atoms();
        let column_arc = Arc::new(column.to_owned());
        // Frozen whole (terms, phrases and groups together): the body
        // below reads all three lists of each.
        let set_arc = Arc::new(match_set);
        let neg_arc = Arc::new(negatives);
        let units: Vec<(Arc<SuperfileEntry>, ())> = kept.into_iter().map(|e| (e, ())).collect();
...
impl SupertableReader {
    ...
    /// `opts` carries the mode, the statistics, an optional parameter
    /// override, and an optional per-call query expansion that overrides
    /// the column's registered one.
    pub fn bm25_search(
        &self,
        column: &str,
        query: &str,
...
impl Supertable {
    ...
    ///
    /// `opts` carries the boolean mode, the corpus statistics, an
    /// optional per-query similarity-parameter override, and an optional
    /// per-call query expansion (stop terms and term groups). A call
    /// with no expansion applies the one registered for `column` through
