...
/// Object-store URLs the surviving segments are registered under for the
/// duration of a scan. Authorities are arbitrary — they are only keys into
/// the session's object-store registry.
const SUPERFILE_STORE_URL: &str = "superfile://supertable/";
...
impl TableProvider for SupertableProvider {
    ...
    async fn scan(
        ...
        let url = ObjectStoreUrl::parse(SUPERFILE_STORE_URL)?;
        state
            .runtime_env()
