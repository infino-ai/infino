...
mod tests {
    ...
    /// Supertable-level repro of the post-drain RaBitQ-shortlist collapse:
    /// ingest clustered vectors across MANY commits (→ many user superfiles),
    /// then compare r=1 recall pre-drain (user-table path) vs post-drain (cell
    /// fanout path), plus r=256 post-drain. EC2 showed post-drain r=1 cratering
    /// (0.030) while r=256 held (0.985) — the cross-superfile fanout over cells
    /// is the only untested layer. This exercises exactly that.
    ///
    /// OPEN BUG (ignored): reproduces the collapse — pre r=1≈0.99, post r=1≈0.28,
    /// post r=256≈0.93. Build is exonerated (see the builder-side guard test);
    /// the defect is in the cross-superfile fanout's 1-bit shortlist over cells.
    /// Un-ignore + flip the assertion once fixed.
    #[ignore = "repro of open post-drain RaBitQ-shortlist bug; run explicitly with --ignored"]
    #[test]
    fn post_drain_rabitq_shortlist_supertable() {
        use std::sync::Arc;
        ...
        let pool = Arc::new(
            rayon::ThreadPoolBuilder::new().num_threads(2).build().expect("pool"),
        );
        let dir = TempDir::new().expect("tempdir");
        let storage: Arc<dyn StorageProvider> =
            Arc::new(LocalFsStorageProvider::new(dir.path()).expect("provider"));
        ...
        for ci in 0..commits {
            let lo = ci * per;
            let titles: Vec<String> = (lo..lo + per).map(|i| i.to_string()).collect();
            let mut flat = Vec::with_capacity(per * dim);
            for i in lo..lo + per {
                flat.extend_from_slice(&vecs[i]);
            }
        ...
        let k = 10usize;
        let nq = 24usize;
        let recall = |nprobe: usize, rerank: usize| -> f32 {
            let mut tot = 0.0f32;
            for qi in 0..nq {
                let q = &vecs[qi * (n / nq)];
                let mut d: Vec<(usize, f32)> = (0..n)
                    .map(|j| {
                        let dot: f32 = q.iter().zip(&vecs[j]).map(|(x, y)| x * y).sum();
                        (j, 1.0 - dot)
                    })
                    .collect();
                d.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());
                let truth: std::collections::HashSet<usize> =
                    d[..k].iter().map(|(j, _)| *j).collect();
                let batches = st
                    .reader()
                    .vector_search(
                        "emb",
                        q,
                        k,
                        VectorSearchOptions::new().with_nprobe(nprobe).with_rerank_mult(rerank),
                        None,
                        Some(&["title"]),
                    )
                    .expect("search");
                let mut got = 0usize;
                for b in &batches {
                    let col = b.column_by_name("title").expect("title col");
                    let titles =
                        col.as_any().downcast_ref::<LargeStringArray>().expect("largeutf8");
                    for r in 0..titles.len() {
                        let idx: usize = titles.value(r).parse().expect("title idx");
                        if truth.contains(&idx) {
                            got += 1;
                        }
                    }
                }
                tot += got as f32 / k as f32;
            }
            tot / nq as f32
        };

        let pre_r1_np4 = recall(4, 1);
        let pre_r1_np64 = recall(64, 1);
        st.drain_vectors_to_cells_sync().expect("drain");
        let post_r1_np4 = recall(4, 1);
        let post_r1_np64 = recall(64, 1);
        let post_r256 = recall(64, 256);
        eprintln!(
            "[repro-st] PRE  r1/np4={pre_r1_np4:.3} r1/np64={pre_r1_np64:.3}  |  POST r1/np4={post_r1_np4:.3} r1/np64={post_r1_np64:.3} r256/np64={post_r256:.3}"
        );
        // EC2 signature: post r=256 holds, post r=1 collapses far below pre r=1.
        assert!(pre_r1_np64 > 0.5, "pre-drain r=1 should be reasonable, got {pre_r1_np64}");
        assert!(
            post_r1_np4 > 0.5 * pre_r1_np4,
            "REPRO: post-drain r=1 collapsed: pre(np4)={pre_r1_np4:.3} post(np4)={post_r1_np4:.3} post_r256={post_r256:.3}"
        );
    }
