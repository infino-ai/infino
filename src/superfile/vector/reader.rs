...
impl VectorReader {
    ...
    /// Resolve stable `_id`s from an inline region when the subsection carries
    /// one (hidden-cell materialized builds). Returns `None` when absent or not
    /// resident — callers fall back to the scalar `_id` column.
