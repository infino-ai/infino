...
- `**Connection`** — `create_table`, `open_table`, `drop_table` (purges the table's storage by default; `purge = false` only unregisters it from the catalog and leaves the bytes), `list_tables`, `query_sql`.
