...
│   ├── cell_grid.rs       ← global vector cell grid (routing, replication, split) for the hidden index
...
| Hidden vector cell grid / drain maintenance   | `src/supertable/cell_grid.rs` + hidden-index path in `writer.rs`      |
...
- **In-place IVF rebalance.** Superfiles are immutable by design. Updates = delete + insert via tombstones.
- **In-place mutation of a committed superfile's bytes.** Superfiles are immutable by design; the user table stays append-only and time-ordered. Updates to user data = delete + insert via tombstones. (This is *not* a ban on hidden-index maintenance — see below — only on rewriting committed user-superfile bytes in place.)
...
#### Hidden vector cell index (implemented — was previously on this list)

In-place IVF rebalance used to be rejected outright. The hidden index now ships as an **MVCC, immutable-superfile** design that does *not* mutate anything in place — so the immutability principle above is preserved:

- Vector search runs over a **hidden, derived vector-index supertable** (a cell-ordered acceleration layer) alongside the immutable, time-ordered user table. User superfiles are vector staging; the hidden manifest holds per-cell IVF output after drain.
- The global cell grid (`src/supertable/cell_grid.rs`) plus the hidden-index path in `writer.rs`/`handle.rs` handles encoded-domain routing (SPANN replication), manifest centroid/count/radius updates, and overflow split (Sq8+ε k-means, N→N+1 centroids). Drain and compaction express logical updates as **physical append + MVCC swap** — merge small per-cell superfiles toward a packed base via `compaction::merge_superfiles`, then atomically swap the manifest; never edit existing bytes.
- All split/reassign math stays on stored **Sq8+ε** bytes (`distance_encoded_to_centroid`, `route_and_splice_ivf_subsections`), never a full-corpus fp32 decode.
