...
hot = warm steady state: every segment mmap-promoted via the public `Supertable::wait_until_warm` before timing, so reads hit resident pages (no object-store GETs). cold = fresh disk cache → object-store range GETs (the backend selected by `INFINO_BENCH_STORE`: s3s-fs, S3, or Azure), excluding the one-time manifest open.
