...
fn open_reader_from_blob(blob: Vec<u8>) -> VectorReader {
    let n_cent = corpus::n_cent(N_DOCS);
    let json = format!(
        r#"[{{"name":"v","dim":{},"n_cent":{},"rot_seed":7,"metric":"cosine"}}]"#,
        corpus::DIM,
        n_cent
    );
...
fn recall_ids(
    ...
    for (q, truth) in queries.iter().zip(truths) {
        let hits = reader
            .search("v", q, TOP_K, nprobe, rerank_mult)
            .expect("search");
...
#[test]
fn compare_fp32_and_sq8_on_1m_bench_workload() {
    eprintln!(
    ...
    for (nprobe, rerank_mult, label) in configs {
        let recall = recall_ids(&fp32_reader, &queries, &truths, nprobe, rerank_mult);
        let hits = hit_count(&fp32_reader, &queries, &truths, nprobe, rerank_mult);
        eprintln!(
    ...
    for codec in [RerankCodec::Bf16, RerankCodec::Sq8] {
        eprintln!("\ndebug: building {codec:?} reader");
        let reader = build_reader(vectors, codec);
        for (nprobe, rerank_mult, label) in configs {
            let recall = recall_ids(&reader, &queries, &truths, nprobe, rerank_mult);
            let hits = hit_count(&reader, &queries, &truths, nprobe, rerank_mult);
            eprintln!(
...
#[test]
fn inspect_sq8_miss_geometry_on_1m_bench_workload() {
    eprintln!(
    ...
    let sq8 = build_reader(vectors, RerankCodec::Sq8);
    ...
    for (qi, q) in queries.iter().enumerate() {
        ...
        let pred = sq8
            .search("v", q, TOP_K, corpus::n_cent(N_DOCS), 1024)
            .expect("sq8 search");
...
#[test]
fn sq8_oversampled_exact_rescore_recall_on_1m_bench_workload() {
    eprintln!(
    ...
    for m in [10usize, 20, 50, 100, 200] {
        ...
        for (q, truth) in queries.iter().zip(truths.iter()) {
            let truth_set: std::collections::HashSet<u32> = truth.iter().copied().collect();
            let sq8_hits_m = sq8
                .search("v", q, m, corpus::n_cent(N_DOCS), 1024)
                .expect("sq8 oversampled search");
...
#[test]
fn sq8_top20_cheap_sidecar_rescore_on_1m_bench_workload() {
    eprintln!(
    ...
    for (q, truth) in queries.iter().zip(truths.iter()) {
        let truth_set: std::collections::HashSet<u32> = truth.iter().copied().collect();
        let top20 = sq8
            .search("v", q, 20, corpus::n_cent(N_DOCS), 1024)
            .expect("sq8 top20 search");
...
#[test]
fn sq8_top20_residual_sidecar_rescore_on_1m_bench_workload() {
    eprintln!(
    ...
    let sq8_reader = build_reader(vectors, RerankCodec::Sq8);
