...
fn end_to_end_summary_per_column() {
    ...
    let (text_dim, _ts, _to, _trow, _tn, text_radius) =
        r.summary("text_emb").expect("vector summary");
    assert_eq!(text_dim as usize, TEXT_EMB_DIM);
    assert!(text_radius >= 0.0);

    let (img_dim, _is, _io, _irow, _in, img_radius) =
        r.summary("image_emb").expect("vector summary");
